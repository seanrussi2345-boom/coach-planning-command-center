...
export function saveWorkspace(workspace, storage = globalThis.localStorage) { storage?.setItem(STORAGE_KEY, JSON.stringify(workspace)); }
export function clearWorkspace(storage = globalThis.localStorage) { storage?.removeItem(STORAGE_KEY); }
export function downloadWorkspace(workspace) {
  ...
  link.href = url; link.download = `${teamSlug || "team"}-three-phase-hq-${date}.json`;
  document.body.append(link); link.click(); link.remove(); URL.revokeObjectURL(url);
}
export async function readWorkspaceFile(file) { if (!file) throw new Error("Choose a JSON backup file first."); return normalizeWorkspace(JSON.parse(await file.text())); }
